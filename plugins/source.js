(function($)
{
	$.Richeditorio.prototype.source = function()
	{
		return {
			init: function()
			{
			...
			toggle: function()
			{
                ...
                {
                    this.source.show();
                    this.source.$textarea.on('keyup.richeditorio-source', $.proxy(function()
                    {
			...
			setCaretOnShow: function()
			{
				...
				var $selectionMarkers = $editorDiv.find("span.richeditorio-selection-marker");
			...
			hide: function()
			{
				...
				this.source.$textarea.off('.richeditorio-source');

				var code = this.source.$textarea.val();

				code = this.paragraphize.load(code);
				code = this.source.setCaretOnHide(code);
				code = code.replace('&amp;<span id="selection-marker-1" class="richeditorio-selection-marker">​</span>', '<span id="selection-marker-1" class="richeditorio-selection-marker">​</span>&amp;');

(function($)
{
	$.Richeditorio.prototype.properties = function()
	{
		return {
			langs: {
				de: {
			...
			{
				 return String()
				 + '<div class="modal-section" id="richeditorio-modal-properties">'
				 	+ '<section>'
					 ...
					 + '</section>'
					+ '<section>'
						+ '<button id="richeditorio-modal-button-action">' + this.lang.get('properties_save') + '</button>'
						+ '<button id="richeditorio-modal-button-cancel">' + this.lang.get('properties_abort') + '</button>'
					+ '</section>'
				 + '</div>';
			},
			setup: function()
			...
			createLabelId: function(css)
			{
				...
				this.properties.labelId = $('<span />').attr('id', 'richeditorio-properties-label-id-' + this.uuid).attr('title', 'ID').hide();
			...
			createLabelClass: function(css)
			{
				...
				this.properties.labelClass = $('<span />').attr('id', 'richeditorio-properties-label-class-' + this.uuid).attr('title', 'class').hide();
			...
			setEvents: function()
			{
				this.core.element().on('click.callback.richeditorio', this.properties.showOnClick);
				$(document).on('mousedown.richeditorio-properties', $.proxy(this.properties.hideOnBlur, this));

				this.core.element().on('destroy.callback.richeditorio', $.proxy(function()
				{
					$(document).off('.richeditorio-properties');
			...
			showOnClick: function(e)
			{
				...
				if (!this.properties.block || !this.utils.isRicheditorioParent(this.properties.block) || this.utils.isCurrentOrParent(['figure', 'li']))
				{
					return;
				}

/*
	...
	Inspired by Richeditorio

	Usage: $('#content').richeditorio();
*/
...
(function($)
{
	...
	// Plugin
	$.fn.richeditorio = function(options)
	{
		var val = [];
		...
		if (typeof options === 'string')
		{
			this.each(function()
			{
				var instance = $.data(this, 'richeditorio');
				var func;
				...
				{
					$.error('No such method "' + options + '" for Richeditorio');
				}
			});
		}
		else
		{
			this.each(function()
			{
				$.data(this, 'richeditorio', {});
				$.data(this, 'richeditorio', Richeditorio(this, options));
			});
		}
	...
	// Initialization
	function Richeditorio(el, options)
	{
		return new Richeditorio.prototype.init(el, options);
	}

	// Options
	$.Richeditorio = Richeditorio;
	$.Richeditorio.VERSION = '2.8.1';
	$.Richeditorio.modules = ['air', 'autosave', 'block', 'buffer', 'build', 'button', 'caret', 'clean', 'code', 'core', 'detect', 'dropdown',
						  'events', 'file', 'focus', 'figure', 'image', 'indent', 'inline', 'insert', 'keydown', 'keyup',
	...
	$.Richeditorio.settings = {};
	$.Richeditorio.opts = {

		// settings
        classPrefix: 'richeditorio',
		animation: false,
		lang: 'en',
		direction: 'ltr',
		spellcheck: true,
		...
		buttons: ['format', 'bold', 'italic', 'deleted', 'lists', 'image', 'file', 'link'], // + 'underline', 'ol', 'ul', 'indent', 'outdent'
	...
	// Functionality
	Richeditorio.fn = $.Richeditorio.prototype = {
		...
		init: function(el, options)
		{
			...
			this.loadModules();
			this.initializeAviary();

			// click to edit
			if (this.opts.clickToEdit && !this.$element.hasClass('richeditorio-click-to-edit'))
			{
				return this.loadToEdit(options);
			}
			else if (this.$element.hasClass('richeditorio-click-to-edit'))
			{
				this.$element.removeClass('richeditorio-click-to-edit');
			}
		...
		loadToEdit: function(options)
		{
			...
			this.$element.on('click.richeditorio-click-to-edit', $.proxy(function()
			{
				this.initToEdit(options);

			}, this));

			this.$element.addClass('richeditorio-click-to-edit');
		...
		initToEdit: function(options)
		{
			$.extend(options.callbacks,  {
				...
				initToEdit: function()
				{
					...
					// cancel
					$(this.opts.clickToCancel).off('.richeditorio-click-to-edit');
					$(this.opts.clickToCancel).show().on('click.richeditorio-click-to-edit', $.proxy(function(e)
					{
						e.preventDefault();
					...
					// save
					$(this.opts.clickToSave).off('.richeditorio-click-to-edit');
					$(this.opts.clickToSave).show().on('click.richeditorio-click-to-edit', $.proxy(function(e)
					{
						e.preventDefault();
			...
			this.$element.richeditorio(options);
			this.$element.off('.richeditorio-click-to-edit');
		...
		loadOptions: function(options)
		{
			...
			// check namespace
			if (typeof $.Richeditorio.settings.namespace !== 'undefined')
			{
				 if (this.$element.hasClass($.Richeditorio.settings.namespace))
				 {
					 settings = $.Richeditorio.settings;
				 }
			}
			else
			{
		...
		{
			var len = $.Richeditorio.modules.length;
			for (var i = 0; i < len; i++)
			{
				this.bindModuleMethods($.Richeditorio.modules[i]);
			}
		},
		...
		initializeAviary: function()
		{
			this.featherEditor = new Aviary.Feather(
			{
                ...
                onSave: function(imageID, newURL) 
                {
					...
					$('#'+imageID).closest('figure.'+this.opts.classPrefix+'-figure').addClass(this.opts.classPrefix+'-process');
					$('#'+imageID).load($.proxy(function(){
						$('#'+imageID).closest('figure.'+this.opts.classPrefix+'-figure').removeClass(this.opts.classPrefix+'-process');
					}, this));
			    }.bind(this),
		...
		air: function()
		{
			return {
				...
				build: function()
				{
					...
					this.core.editor().on('mouseup.richeditorio', this, $.proxy(function(e)
					{
				...
				{
					return $('<ul>').addClass('richeditorio-air').attr({ 'id': 'richeditorio-air-' + this.uuid, 'role': 'toolbar' }).hide();
				},
				show: function (e)
				{
					//this.marker.remove();
				...
				bindHide: function()
				{
					$(document).on('mousedown.richeditorio-air.' + this.uuid, $.proxy(function(e)
					{
						var dropdown = $(e.target).closest('.richeditorio-dropdown').length;
					...
					}, this)).on('keydown.richeditorio-air.' + this.uuid, $.proxy(function(e)
					{
						var key = e.which;
						if ((!this.utils.isRicheditorioParent(e.target) && !$(e.target).hasClass('richeditorio-in')) || $(e.target).closest('#richeditorio-modal').length !== 0)
						{
							return;
						}
				...
				hide: function(e)
				{
					...
					$(document).off('mousedown.richeditorio-air.' + this.uuid);
					$(document).off('keydown.richeditorio-air.' + this.uuid);
		...
		block: function()
		{
			return {
				...
				formatUncollapsed: function(tag, attr, value, type)
				{
                    ...
                    if (blocks[0] && ($(blocks[0]).hasClass('richeditorio-in') || $(blocks[0]).hasClass('richeditorio-box')))
                    {
				...
				removeInlineTags: function(node)
				{
					...
					$(node).find(tags.join(',')).not('.richeditorio-selection-marker').contents().unwrap();
				},
				...
				getBlocks: function(block)
				{
    				...
    				if ($(block).hasClass('richeditorio-box'))
    				{
        				var blocks = [];
		...
		build: function()
		{
			return {
				...
				{
					this.$box = $('<div class="richeditorio-box" role="application" />');
				},
				setIn: function()
				{
					this.core.editor().addClass('richeditorio-in');
				},
				setId: function()
				{
					var id = (this.opts.type === 'textarea') ? 'richeditorio-uuid-' + this.uuid : this.$element.attr('id');

					this.core.editor().attr('id', (typeof id === 'undefined') ? 'richeditorio-uuid-' + this.uuid : id);
				},
				getName: function()
				{
				...
				loadFromTextarea: function()
				{
                    ...
                    this.$editor.addClass('richeditorio-layer');

					if (this.opts.overrideStyles)
					{
					    this.$editor.addClass('richeditorio-styles');
					}

					this.$element.hide();

					this.$box.prepend('<span class="richeditorio-voice-label" id="richeditorio-voice-' + this.uuid +'" aria-hidden="false">' + this.lang.get('accessibility-help-label') + '</span>');
					this.$editor.attr({ 'aria-labelledby': 'richeditorio-voice-' + this.uuid, 'role': 'presentation' });
				},
				...
				setOptions: function()
				{
					...
					{
						this.core.editor().addClass('richeditorio-structure');
					}
				...
				callEditor: function()
				{
					...
					{
						this.core.editor().on('mouseup.richeditorio-observe.' + this.uuid + ' keyup.richeditorio-observe.' + this.uuid + ' focus.richeditorio-observe.' + this.uuid + ' touchstart.richeditorio-observe.' + this.uuid, $.proxy(this.observe.toolbar, this));
						this.core.element().on('blur.callback.richeditorio', $.proxy(function()
						{
							this.button.setInactiveAll();
				...
				plugins: function(s)
				{
                    var func = (typeof RicheditorioPlugins !== 'undefined' && typeof RicheditorioPlugins[s] !== 'undefined') ? RicheditorioPlugins : Richeditorio.fn;
		...
		button: function()
		{
			return {
				...
				buildButtonTooltip: function($btn, title)
				{
                    ...
                    {
                        if ($(this).hasClass('richeditorio-button-disabled'))
                        {
                            return;
                        }
				...
				build: function(btnName, btnObject)
				{
					...
					{
						$button.addClass('richeditorio-toolbar-link-dropdown').attr('aria-haspopup', true);

						var $dropdown = $('<ul class="richeditorio-dropdown richeditorio-dropdown-' + this.uuid + ' richeditorio-dropdown-box-' + btnName + '" style="display: none;">');
				...
				setEvent: function($button, btnName, btnObject)
				{
					$button.on('mousedown', $.proxy(function(e)
					{
						...
						if ($button.hasClass('richeditorio-button-disabled'))
						{
							return false;
						}
				...
				addCallback: function($btn, callback)
				{
					...
					{
						if ($btn.hasClass('richeditorio-button-disabled'))
						{
							return false;
						}
				...
				addDropdown: function($btn, dropdown)
				{
					...
					$btn.addClass('richeditorio-toolbar-link-dropdown').attr('aria-haspopup', true);

					var key = $btn.attr('rel');
					this.button.addCallback($btn, 'dropdown');

					var $dropdown = $('<div class="richeditorio-dropdown richeditorio-dropdown-' + this.uuid + ' richeditorio-dropdown-box-' + key + '" style="display: none;">');
				...
				{
					this.button.get(key).addClass('richeditorio-act');
                    this.button.getDropdown(key).closest('ul').find('a').removeClass('selected');
                    this.button.getDropdown(key).addClass('selected');
				},
                getDropdown: function(key)
                {
                    ...
                    return $(document).find('ul.richeditorio-dropdown-'+ this.uuid +'.richeditorio-dropdown-box-format a[rel="'+ key +'"]');
                },
				setInactive: function(key)
				{
					this.button.get(key).removeClass('richeditorio-act');
				},
				setInactiveAll: function(key)
				{
					...
					$btns.removeClass('richeditorio-act');
				},
				disable: function(key)
				{
					this.button.get(key).addClass('richeditorio-button-disabled');
				},
				enable: function(key)
				{
					this.button.get(key).removeClass('richeditorio-button-disabled');
				},
				disableAll: function(key)
				{
					...
					$btns.addClass('richeditorio-button-disabled');
				},
				enableAll: function()
				{
					this.button.toolbar().find('a.re-button').removeClass('richeditorio-button-disabled');
				},
				remove: function(key)
				{
		...
		clean: function()
		{
			return {
				onSet: function(html)
				{
					...
					if (this.opts.script)
					{
						html = html.replace(/<script(.*?[^>]?)>([\w\W]*?)<\/script>/gi, '<pre class="richeditorio-script-tag" $1>$2</pre>');
					}
					...
					// add span marker
					$div.find('span, a').attr('data-richeditorio-span', true);
					...
					$div.find(this.opts.inlineTags.join(',')).each(function()
					{
    					...
    					if ($el.attr('style'))
    					{
        					$el.attr('data-richeditorio-style-cache', $el.attr('style'));
    					}
					});
					...
					// restore markers
					html = html.replace('###marker1###', '<span id="selection-marker-1" class="richeditorio-selection-marker">​</span>');
					html = html.replace('###marker2###', '<span id="selection-marker-2" class="richeditorio-selection-marker">​</span>');
				...
				onSync: function(html)
				{
					...
					// remove image resize
					html = html.replace(/<span(.*?)id="richeditorio-figure-box"(.*?[^>])>([\w\W]*?)<img(.*?)><\/span>/gi, '$3<img$4>');
					html = html.replace(/<span(.*?)id="richeditorio-figure-resizer"(.*?[^>])>(.*?)<\/span>/gi, '');
					html = html.replace(/<span(.*?)id="richeditorio-figure-options"(.*?[^>])>(.*?)<\/span>/gi, '');
					...
					$div.find('*[data-richeditorio-style-cache]').removeAttr('data-richeditorio-style-cache');

					// remove markers
					$div.find('.richeditorio-invisible-space, .richeditorio-unlink').each(function()
					{
						$(this).contents().unwrap();
					});

					// remove span without attributes & span marker
				    $div.find('span, a').removeAttr('data-richeditorio-span data-richeditorio-style-cache').each(function()
					{
					...
					$div.find('.richeditorio-selection-marker, #richeditorio-insert-marker').remove();
					...
					if (this.opts.script)
					{
						html = html.replace(/<pre class="richeditorio-script-tag"(.*?[^>]?)>([\w\W]*?)<\/pre>/gi, '<script$1>$2</script>');
					}
				...
				convertTags: function(html, data)
				{
					...
					for (var i = 0; i < len; i++)
					{
						...
						else if (this.utils.isInlineTag(tags[i]))
						{
                            ...
                            html = html.replace(new RegExp('<' + tags[i] + '([^>]*)data-richeditorio-style-cache="([^>]*)"[^>]*>', 'gi'), '###' + tags[i] + ' cache="$2"###');
                            html = html.replace(new RegExp('<' + tags[i] + '[^>]*>', 'gi'), '###' + tags[i] + '###');
						}
						else
						{
				...
				reconvertTags: function(html, data)
				{
					...
					for (var i = 0; i < len; i++)
					{
                        ...
                        else if (this.utils.isInlineTag(tags[i]))
                        {
                            ...
                            var spanMarker = (tags[i] === 'span') ? ' data-richeditorio-span="true"' : '';

                            html = html.replace(new RegExp('###' + tags[i] + ' cache="(.*?[^#])"###', 'gi'), '<' + tags[i] + ' style="$1"' + spanMarker + ' data-richeditorio-style-cache="$1">');
                            html = html.replace(new RegExp('###' + tags[i] + '\s?(.*?[^#])###', 'gi'), '<' + tags[i] + '$1>');
                        }
                    }
				...
				{
					html = html.replace(/&lt;span id=&quot;selection-marker-([0-9])&quot; class=&quot;richeditorio-selection-marker&quot;&gt;​&lt;\/span&gt;/g, '<span id="selection-marker-$1" class="richeditorio-selection-marker">​</span>');

					return html;
				},
				saveFormTags: function(html)
				{
					return html.replace(/<form(.*?)>([\w\W]*?)<\/form>/gi, '<section$1 rel="richeditorio-form-tag">$2</section>');
				},
				restoreFormTags: function(html)
				{
					return html.replace(/<section(.*?) rel="richeditorio-form-tag"(.*?)>([\w\W]*?)<\/section>/gi, '<form$1$2>$3</form>');
				},
				...
				{
					return html.replace(/<span(.*?[^>]?)class="richeditorio-selection-marker"(.*?[^>]?)>([\w\W]*?)<\/span>/gi, '');
				},
		...
		code: function()
		{
			return {
				...
				{
					html = $.trim(html);
					html = html.replace(/^(<span id="selection-marker-1" class="richeditorio-selection-marker">​<\/span>)/, '');
					...
					html = html.replace(/<p><span id="selection-marker-1" class="richeditorio-selection-marker">​<\/span><\/p>/, '');
		...
		core: function()
		{
			return {
				...
				structure: function()
				{
					this.core.editor().toggleClass('richeditorio-structure');
				},
				...
				{
					var eventNamespace = 'richeditorio';
				...
				destroy: function()
				{
					...
					// help label
					$('#richeditorio-voice-' + this.uuid).remove();

					this.core.editor().removeClass('richeditorio-in richeditorio-styles richeditorio-structure richeditorio-figure-layer-edit');

					// caret service
					this.core.editor().off('keydown.richeditorio-remove-textnode');

					// observer
					this.core.editor().off('.richeditorio-observe.' + this.uuid);

					// off events and remove data
					this.$element.off('.richeditorio').removeData('richeditorio');
					this.core.editor().off('.richeditorio');

					$(document).off('.richeditorio-dropdown');
					$(document).off('.richeditorio-air.' + this.uuid);
					$(document).off('mousedown.richeditorio-blur.' + this.uuid);
					$(document).off('mousedown.richeditorio.' + this.uuid);
					$(document).off('touchstart.richeditorio.' + this.uuid + ' click.richeditorio.' + this.uuid);
					$(window).off('.richeditorio-toolbar.' + this.uuid);
					$(window).off('touchmove.richeditorio.' + this.uuid);
					$("body").off('scroll.richeditorio.' + this.uuid);

					$(this.opts.toolbarFixedTarget).off('scroll.richeditorio.' + this.uuid);
					...
					if (this.opts.plugins !== false)
					{
						...
						{
							$(window).off('.richeditorio-plugin-' + s);
							$(document).off('.richeditorio-plugin-' + s);
							$("body").off('.richeditorio-plugin-' + s);
							self.core.editor().off('.richeditorio-plugin-' + s);
						});
					}

					// click to edit
					this.$element.off('click.richeditorio-click-to-edit');
					this.$element.removeClass('richeditorio-click-to-edit');

					// common
					this.core.editor().removeClass('richeditorio-layer');
					...
					// hide link's tooltip
					$('.richeditorio-link-tooltip').remove();
		...
		dropdown: function()
		{
			return {
				...
				buildItem: function(btnName, btnObject)
				{
					...
					{
    					$itemContainer.addClass('richeditorio-dropdown-divider');

    					return $itemContainer;
					}
					var $item = $('<a href="#" class="richeditorio-dropdown-' + btnName + '" role="button" rel="'+ btnObject.rel +'"/>');
				...
				{
					if ($(e.target).hasClass('richeditorio-dropdown-link-inactive'))
					{
						return;
					}
				...
				show: function(e, key)
				{
					...
					{
						this.dropdown.active.on('mouseover.richeditorio-dropdown', $.proxy(this.utils.disableBodyScroll, this));
						this.dropdown.active.on('mouseout.richeditorio-dropdown mousedown.richeditorio-dropdown', $.proxy(this.utils.enableBodyScroll, this));
					}
				...
				showIsFixedToolbar: function()
				{
					...
					// animate
					this.dropdown.active.richeditorioAnimation('slideDown', { duration: 0.2 }, $.proxy(function()
					{
				...
				showIsUnFixedToolbar: function()
				{
					...
					// animate
					this.dropdown.active.richeditorioAnimation(((this.opts.animation) ? 'slideDown' : 'show'), { duration: 0.2 }, $.proxy(function()
					{
						this.dropdown.enableCallback();
						this.dropdown.enableEvents();
				...
				{
					$(document).on('mousedown.richeditorio-dropdown', $.proxy(this.dropdown.hideAll, this));
					this.core.editor().on('touchstart.richeditorio-dropdown', $.proxy(this.dropdown.hideAll, this));
					$(document).on('keyup.richeditorio-dropdown', $.proxy(this.dropdown.closeHandler, this));
				},
				...
				hideAll: function(e, key)
				{
					...
					if (e !== false && $(e.target).closest('.richeditorio-dropdown').length !== 0)
					{
						return;
					}

					var $buttons = (typeof key === 'undefined') ? this.button.toolbar().find('a.dropact') : this.button.toolbar().find('a.dropact').not('.re-' + key);
					var $elements = (typeof key === 'undefined') ? $('.richeditorio-dropdown-' + this.uuid) : $('.richeditorio-dropdown-' + this.uuid).not('.richeditorio-dropdown-box-' + key);

					if ($elements.length !== 0)
					{
						$(document).off('.richeditorio-dropdown');
						this.core.editor().off('.richeditorio-dropdown');
						...
						$.each($elements, $.proxy(function(i,s)
						{
							...
							$el.hide();
							$el.off('mouseover mouseout').off('.richeditorio-dropdown');

						}, this));

						$buttons .removeClass('richeditorio-act dropact');
					}
				...
				hide: function ()
				{
					...
					this.dropdown.active.richeditorioAnimation(((this.opts.animation) ? 'slideUp' : 'hide'), { duration: 0.2 }, $.proxy(function()
					{
						$(document).off('.richeditorio-dropdown');
						this.core.editor().off('.richeditorio-dropdown');
				...
				hideOut: function()
				{
					...
					this.dropdown.button.removeClass('richeditorio-act dropact');
					this.dropdown.active.off('mouseover mouseout').off('.richeditorio-dropdown');
		...
		events: function()
		{
			return {
				...
				dragover: function(e)
				{
					...
					{
						$(e.target).addClass('richeditorio-figure-dragover');
					}
				...
				dragleave: function(e)
				{
					...
					this.core.editor().find('img').removeClass('richeditorio-figure-dragover');
				},
				drop: function(e)
				{
					e = e.originalEvent || e;
				...
				blur: function(e)
				{
					...
					if ($(e.target).closest('#' + this.core.id() + ', .richeditorio-toolbar, .richeditorio-dropdown, #richeditorio-modal-box').length !== 0)
					{
						return;
					}
				...
				touchImageEditing: function()
				{
					...
					$(window).on('touchmove.richeditorio.' + this.uuid, $.proxy(function()
					{
				...
				init: function()
				{
					this.core.editor().on('dragover.richeditorio dragenter.richeditorio', $.proxy(this.events.dragover, this));
					this.core.editor().on('dragleave.richeditorio', $.proxy(this.events.dragleave, this));
					this.core.editor().on('drop.richeditorio', $.proxy(this.events.drop, this));
					this.core.editor().on('click.richeditorio', $.proxy(this.events.click, this));
					this.core.editor().on('paste.richeditorio', $.proxy(this.paste.init, this));
					this.core.editor().on('keydown.richeditorio', $.proxy(this.keydown.init, this));
					this.core.editor().on('keyup.richeditorio', $.proxy(this.keyup.init, this));
					this.core.editor().on('focus.richeditorio', $.proxy(this.events.focus, this));

					$(document).on('mousedown.richeditorio-blur.' + this.uuid, $.proxy(this.events.blur, this));
		...
		file: function()
		{
			return {
				...
				show: function()
				{
					...
					// build upload
					this.upload.init('#richeditorio-modal-file-upload', this.opts.fileUpload, this.file.insert);

					// set selected text
					$('#richeditorio-filename').val(this.selection.get().toString());
				...
				text: function(json)
				{
					var text = $('#richeditorio-filename').val();
        ...
        figure: function()
        {
            return {
                ...
                {
                    var $figure =  $('<figure class="'+this.opts.classPrefix+'-figure '+ type +'Figure" contenteditable="false">');
                    $figure.attr('data-type', type);
                    if(type === 'image')
                    {
                        var $img = $('<img>');
						$img.attr('src', src).attr('id', this.getRandomId('image'));
						$img.addClass('imageSrc');
						$figure.append($img);
                    }
					if(type === 'embedcode')
					{
						var $span = $('<span class="embed-span">'+src+'</span>');
						//$span.attr('text',src);
						$figure.append($span);
					}
                    return $figure;
                ...
                setEditable: function($figure)
                {
                    ...
                    if (this.opts.imageResizable)
                    {
                        var handler = $.proxy(function(e)
                        {
                            ...
                            $(document).on('mousedown.richeditorio-figure-resize-hide.' + this.uuid, $.proxy(this.figure.hideResize, this));

                            if (this.figure.resizer)
                            {
                                this.figure.resizer.on('mousedown.richeditorio touchstart.richeditorio', $.proxy(function(e)
                                {
                        ...
                        $figure.off('mousedown.richeditorio').on('mousedown.richeditorio', $.proxy(this.figure.hideResize, this));
                        $figure.off('click.richeditorio touchstart.richeditorio').on('click.richeditorio touchstart.richeditorio', handler);
                    }
                    else
                    {
                        $figure.off('click.richeditorio touchstart.richeditorio').on('click.richeditorio touchstart.richeditorio', $.proxy(function(e)
                        {
                            setTimeout($.proxy(function()
                            {
                                this[$figure.attr('data-type')].showEdit($figure);
                ...
                {
                    $(document).on('mousemove.richeditorio-figure-resize touchmove.richeditorio-figure-resize', $.proxy(this.figure.moveResize, this));
                    $(document).on('mouseup.richeditorio-figure-resize touchend.richeditorio-figure-resize', $.proxy(this.figure.stopResize, this));
                },
                moveResize: function(e)
                {
                    ...
                    var width = this.figure.resizeHandle.w;

                    if (e.targetTouches) width += (e.targetTouches[0].pageX -  this.figure.resizeHandle.x);
                    else width += (e.pageX -  this.figure.resizeHandle.x);
                ...
                {
                    this.handle = false;
                    $(document).off('.richeditorio-figure-resize');

                    this.figure.hideResize();
                },
                hideResize: function(e)
                {
                    if (e && $(e.target).closest('#richeditorio-figure-box', this.$editor[0]).length !== 0) return;

                    var figureBox = this.$editor.find('#richeditorio-figure-box');
                    if (figureBox.length === 0) return;

                    $('#richeditorio-figure-options').remove();
                    $('#richeditorio-figure-resizer').remove();
                    this.observe.figure.removeClass('figure-in-edit');

                    figureBox.css('margin', '');
                    figureBox.replaceWith(function()
                    {
                        return $(this).find('#richeditorio-figure-content').contents();
                    });

                    $(document).off('mousedown.richeditorio-figure-resize-hide.' + this.uuid);

                    this.button.enableAll();
                },
                loadResizableControls: function($figure, figureInnerBox)
                {
                    var figureContent = $('<span id="richeditorio-figure-content"></span>');
                    figureContent.append($figure.html());

                    if (this.opts.imageResizable && !this.detect.isMobile())
                    {
                        var figureResizer = $('<span id="richeditorio-figure-resizer" data-richeditorio="verified"></span>');
                        ...
                        figureInnerBox.append(figureResizer);
                        figureInnerBox.append(figureContent);
                        $figure.html(figureInnerBox);
                ...
                {
                    if ($('#richeditorio-figure-box').length !== 0)
                    {
                        return;
                    }
                    this.button.disableAll();

                    $figure.addClass('figure-in-edit');
                    var figureInnerBox = $('<span id="richeditorio-figure-box" data-richeditorio="verified">');
                    figureInnerBox.css({'float': $figure.css('float'), 'outline-color':this.opts.imageSelectedColor, 'box-shadow':'0 0 12px '+this.opts.imageSelectedColor}).attr('contenteditable', false);
                    figureInnerBox.append(this[$figure.attr('data-type')].actions($figure));

                    return this.figure.loadResizableControls($figure, figureInnerBox);

                },
                setFloating: function($figure)
                {
                    var floating = $('#richeditorio-figure-align').val();
                    ...
                    switch (floating)
                    {
                        case 'left':
                            imageFloat = 'left';
                            imageMargin = '0 ' + this.opts.imageFloatMargin + 'px ' + this.opts.imageFloatMargin/2 + 'px 0';
                            break;
                        case 'right':
                            imageFloat = 'right';
                            imageMargin = '0 0 ' + this.opts.imageFloatMargin/2 + 'px ' + this.opts.imageFloatMargin + 'px';
                            break;
                        case 'center':
                            if ($figure.length !== 0)
                            {
                    ...
                    $figure.css({ 'float': imageFloat, 'display': imageDisplay, 'margin': imageMargin, 'text-align': textAlign });
                },
                addCaption: function($figure)
                {
                    var caption = $('#richeditorio-figure-caption').val();
		...
		image: function()
		{
			return {
				...
				show: function()
				{
					...
					// build upload
					this.upload.init('#richeditorio-modal-image-droparea', this.opts.imageUpload, this.image.insert);
					this.modal.show();
				...
				insert: function(json, direct, e)
				{
                    ...
                    var $figure = this.figure.get('image', json.url);
                    $img = $figure.find('img');
                ...
                showEdit: function($figure)
                {
                    ...
                    {
                        $('.richeditorio-image-position-option').hide();
                    }
                    else
                    {
                    ...
                    $('#richeditorio-image-preview').html($('<img src="' + $image.attr('src') + '" style="max-width: 100%;">'));
                    $('#richeditorio-image-title').val($image.attr('alt'));

                    if ($link.length !== 0)
                    {
                        $('#richeditorio-image-link').val($link.attr('href'));
                        if ($link.attr('target') === '_blank')
                        {
                            $('#richeditorio-image-link-blank').prop('checked', true);
                        }
                    }
                    ...
                    if (this.detect.isDesktop())
                    {
                        $('#richeditorio-image-title').focus();
                    }
                ...
                update: function()
                {
                    ...
                    var title = $('#richeditorio-image-title').val().replace(/(<([^>]+)>)/ig,"");
                    $image.attr('alt', title).attr('title', title);

                    this.figure.setFloating($image.closest('figure'));

                    // as link
                    var link = $.trim($('#richeditorio-image-link').val()).replace(/(<([^>]+)>)/ig,"");
                    if (link !== '')
                    {
                        ...
                        var target = ($('#richeditorio-image-link-blank').prop('checked')) ? true : false;

                        if ($link.length === 0)
                        {
                            var a = $('<a href="' + link + '" id="richeditorio-img-tmp">' + this.utils.getOuterHtml($image) + '</a>');
                            if (target)
                            {
                            ...
                            $link = this.core.editor().find('#richeditorio-img-tmp');
                            $link.removeAttr('id');
                        }
                        else
                        {
                    ...
                    this.figure.addCaption($image.closest('figure'));
                    this.modal.close();
                ...
                actions: function($figure)
                {
                    ...
                    // edittor
                    this.image.edittor = $('<li class="richeditorio-editBtn" data-richeditorio="verified">'+ this.lang.get('edit') + '</li>');
                    this.image.edittor.on('click', $.proxy(function(){this.image.showEdit($figure);}, this));
                    editorOptions.append(this.image.edittor);

                    // adobe creative cloud
                    this.image.advance = $('<li class="richeditorio-editBtn" data-richeditorio="verified">'+ this.lang.get('advance-edit') + '</li>');
                    this.image.advance.on('click', $.proxy(function(){return this.image.launchAviaryEditor($image.attr('id'), $image.attr('src'));}, this));
                    editorOptions.append(this.image.advance);

                    // delete image
                    this.image.delete = $('<li class="richeditorio-editBtn" data-richeditorio="verified">'+ this.lang.get('delete') + '</li>');
                    this.image.delete.on('click', $.proxy(function(){this.figure.remove($figure); this.button.enableAll();}, this));
                    editorOptions.append(this.image.delete);

                    return editorOptions;
                },
				launchAviaryEditor: function(id, src)
				{
		...
		inline: function()
		{
			return {
    			...
    			getClearedNodes: function()
    			{
                    ...
                    for (var i = 0; i < len; i++)
					{
						if ($(nodes[i]).hasClass('richeditorio-selection-marker'))
						{
    						started = i + 2;
    						break;
						}
    			...
    			setNodesStriked: function(nodes, tag, params)
    			{
                    for (var i = 0; i < nodes.length; i++)
					{
                        ...
                        if (convertable)
                        {
                            ...
                            $el.attr('data-richeditorio-inline-converted');
                        }
                ...
                insertBreakpoint: function(inline, currentTag)
				{
					...
					breakpoint.id = 'richeditorio-inline-breakpoint';
                    ...
                    code = code.replace(/<span id="richeditorio-inline-breakpoint"><\/span>/i, "</" + currentTag + ">" + endTag);
                    ...
                    if (params)
                    {
                        for (var key in params.data)
                        {
                            ...
                            if (key === 'style')
                            {
                                ...
                                $node.attr('data-richeditorio-style-cache', $node.attr('style'));
                            }
                            else if (key === 'class')
                            {
                            ...
                            {
                                $node.attr('data-richeditorio-span', true);
                            }
                        }
                    }
				...
				removeAttr: function(name, node)
				{
    				return this.inline.eachInline(node, function(el)
    				{
                        ...
                        if (name === 'style')
                        {
        				    $el.removeAttr('data-richeditorio-style-cache');
        				}
				...
				addParentStyle: function($el)
				{
                    ...
                    if ($parent)
                    {
                        ...
                        $parent.css(style);
                        $parent.attr('data-richeditorio-style-cache', $parent.attr('style'));
				...
				removeAllStyle: function(node)
				{
    				return this.inline.eachInline(node, function(el)
    				{
                        return $(el).removeAttr('style').removeAttr('data-richeditorio-style-cache');
    				});
				},
                removeStyleRule: function(name)
				{
                    ...
                    for (var i = 0; i < nodes.length; i++)
					{
    					var el = nodes[i];
						var $el = $(el);
						if ($.inArray(el.tagName.toLowerCase(), this.opts.inlineTags) != -1 && !$el.hasClass('richeditorio-selection-marker'))
						{
		...
		insert: function()
		{
			return {
				...
				placeHtml: function(html)
				{
					...
					marker.id = 'richeditorio-insert-marker';
		...
		keydown: function()
		{
			return {
				init: function(e)
				{
					...
					if ((this.opts.type === 'textarea' || this.opts.type === 'div') && this.keydown.current && this.keydown.current.nodeType === 3 && $(this.keydown.parent).hasClass('richeditorio-in'))
					{
					...
					{
    					if (this.observe.figure && typeof this.observe.figure !== 'undefined' && $('#richeditorio-figure-box').length !== 0)
    					{
        					e.preventDefault();
					...
					if (key === this.keyCode.DELETE)
					{
						...
						if (tagLi)
						{
							...
							if (this.utils.isRicheditorioParent($list) && this.utils.isEndOfElement($list) && $nextList.length !== 0
					...
					// image delete
					if (key === this.keyCode.DELETE && $('#richeditorio-figure-box').length !== 0)
					{
						this.image.remove();
					}
				...
				{
                    if(this.selection.prev() && this.selection.prev().tagName === 'FIGURE') {
                        var $figure = $(this.selection.prev());
                        $figure.click();
                        this.caret.after($figure);
                        e.preventDefault();
                    }
                    // // Fn+Delete image delete
                    // if(this.selection.parent() && this.selection.parent().nextSibling.tagName === 'FIGURE') {
                    //     $figure = $(this.selection.parent().nextSibling);
					...
					setTimeout($.proxy(function()
					{
						...
						$styleTags.not('img, figure, iframe, #richeditorio-figure-box, #richeditorio-figure-options, [data-richeditorio-style-cache], [data-richeditorio-span]' + filter).removeAttr('style');
				...
				replaceToParagraph: function(tag)
				{
					...
					if (blockElem.tagName === tag && this.utils.isEmpty(blockHtml) && !$(blockElem).hasClass('richeditorio-in'))
					{
						var p = document.createElement('p');
			...
			return {
				...
				insert: function(link, cleaned)
				{
					...
					if ($el === false)
					{
						...
						var $parent = $el.parent();
						if (this.utils.isRicheditorioParent($parent) === false)
						{
							$el.wrap('<p>');
						}

						// remove unlink wrapper
						if ($parent.hasClass('richeditorio-unlink'))
						{
				...
				{
					$('#richeditorio-link-blank').prop('checked', link.target);
					$('#richeditorio-link-url').val(link.url);
					$('#richeditorio-link-url-text').val(link.text);
				},
				...
				buildLinkFromModal: function()
				{
					...
					// url
					link.url = this.link.cleanUrl($('#richeditorio-link-url').val());

					// text
					link.text = this.link.cleanText($('#richeditorio-link-url-text').val());
					link.text = this.link.getText(link);

					// target
					link.target = ($('#richeditorio-link-blank').prop('checked')) ? true : false;
				...
				replaceLinksToText: function(links)
				{
					...
					{
						var $el = $(s);
						var $unlinked = $('<span class="richeditorio-unlink" />').append($el.contents());
		...
		linkify: function()
		{
			return {
				...
				format: function()
				{
					...
					this.core.editor().find(":not(iframe,img,a,pre,code,.richeditorio-unlink)").addBack().contents().filter($.proxy(this.linkify.isFiltered, this)).each($.proxy(this.linkify.handler, this));

					// collect
					var $objects = this.core.editor().find('.richeditorio-linkify-object').each($.proxy(function(i,s)
					{
						var $el = $(s);
						$el.removeClass('richeditorio-linkify-object');
						if ($el.attr('class') === '')
						{
							$el.removeAttr('class');
						}
				...
				{
					var iframeStart = '<div class="' + this.opts.videoContainerClass + ' richeditorio-linkify-object"><iframe class="richeditorio-linkify-object" width="500" height="281" src="';
				...
				convertImages: function(html)
				{
					...
					return html.replace(html, '<img src="' + matches + '" class="richeditorio-linkify-object" />');
				},
				convertLinks: function(html)
				{
					...
					for (var i = 0; i < length; i++)
					{
						...
						html = html.replace(regexp, '<a href="' + linkProtocol + $.trim(href) + '"' + target + ' class="richeditorio-linkify-object">' + $.trim(text) + '</a>');
					}
		...
		list: function()
		{
			return {
				toggle: function(cmd)
				{
					...
					var $list = $(this.selection.current()).parentsUntil('.richeditorio-in', 'ul, ol').first();

					this.placeholder.hide();
					this.buffer.set();


					if ($list.length !== 0 && $list[0].tagName === tag && this.utils.isRicheditorioParent($list))
					{
						this.selection.save();
					...
					if (this.utils.isRicheditorioParent($listParent) && $listParent[0].tagName !== 'LI' && this.utils.isBlock($listParent))
					{
						this.selection.save();
		...
		marker: function()
		{
			return {
				...
				get: function(num)
				{
					...
					marker.className = 'richeditorio-selection-marker';
				...
				remove: function()
				{
					this.core.editor().find('.richeditorio-selection-marker').each(this.marker.iterateRemove);
				},
				...
				{
					var parent = this.selection.parent();
					if (range === null || $(parent).closest('.richeditorio-in').length === 0)
					{
						return;
					}
		...
		modal: function()
		{
			return {
				...
				templates: function()
				{
					this.opts.modal = {
						'image-edit': String()
						+ '<div class="richeditorio-modal-tab richeditorio-group" data-title="General">'
							+ '<div id="richeditorio-image-preview" class="richeditorio-modal-tab-side">'
							+ '</div>'
							+ '<div class="richeditorio-modal-tab-area">'
								+ '<section>'
									+ '<label>' + this.lang.get('title') + '</label>'
									+ '<input type="text" id="richeditorio-image-title" />'
								+ '</section>'
								+ '<section>'
								...
								+ '<section>'
									...
									+ '<input type="text" id="richeditorio-image-link" aria-label="' + this.lang.get('link') + '" />'
								+ '</section>'
								+ '<section>'
        							+ '<label class="richeditorio-image-position-option">' + this.lang.get('image-position') + '</label>'
        							+ '<select class="richeditorio-image-position-option" id="richeditorio-figure-align" aria-label="' + this.lang.get('image-position') + '">'
								...
								+ '</section>'
								+ '<section>'
									+ '<label class="checkbox"><input type="checkbox" id="richeditorio-image-link-blank" aria-label="' + this.lang.get('link-in-new-tab') + '"> ' + this.lang.get('link-in-new-tab') + '</label>'
								+ '</section>'
								+ '<section>'
									+ '<button id="richeditorio-modal-button-action">' + this.lang.get('insert') + '</button>'
									+ '<button id="richeditorio-modal-button-cancel">' + this.lang.get('cancel') + '</button>'
								+ '</section>'
							+ '</div>'
						+ '</div>',

						'image': String()
						+ '<div class="richeditorio-modal-tab" data-title="Upload">'
							+ '<section>'
								+ '<div id="richeditorio-modal-image-droparea"></div>'
	 						+ '</section>'
 						+ '</div>',
						...
						+ '<div class="richeditorio-modal-tab" data-title="Upload">'
							+ '<section>'
								...
								+ '<input type="text" id="richeditorio-filename" aria-label="' + this.lang.get('filename') + '" /><br><br>'
							+ '</section>'
							+ '<section>'
								+ '<div id="richeditorio-modal-file-upload"></div>'
							+ '</section>'
						+ '</div>',

						'link': String()
						+ '<div class="richeditorio-modal-tab" data-title="General">'
							+ '<section>'
								+ '<label>URL</label>'
								+ '<input type="url" id="richeditorio-link-url" aria-label="URL" />'
							+ '</section>'
							+ '<section>'
								+ '<label>' + this.lang.get('text') + '</label>'
								+ '<input type="text" id="richeditorio-link-url-text" aria-label="' + this.lang.get('text') + '" />'
							+ '</section>'
							+ '<section>'
								+ '<label class="checkbox"><input type="checkbox" id="richeditorio-link-blank"> ' + this.lang.get('link-in-new-tab') + '</label>'
							+ '</section>'
							+ '<section>'
								+ '<button id="richeditorio-modal-button-action">' + this.lang.get('insert') + '</button>'
								+ '<button id="richeditorio-modal-button-cancel">' + this.lang.get('cancel') + '</button>'
				...
				{
					return this.$modalBody.find('#richeditorio-modal-button-action');
				},
				getCancelButton: function()
				{
					return this.$modalBody.find('#richeditorio-modal-button-cancel');
				},
				getDeleteButton: function()
				{
					return this.$modalBody.find('#richeditorio-modal-button-delete');
				},
				...
				show: function()
				{
					...
					$(window).on('resize.richeditorio-modal', $.proxy(this.modal.buildWidth, this));


					this.$modalOverlay.richeditorioAnimation('fadeIn', {
						duration: 0.25
					});

					this.$modalBox.addClass('open').show();
					this.$modal.richeditorioAnimation('fadeIn', {
						...
						$.proxy(function()
						{
							...
							$elements.on('keydown.richeditorio-modal', $.proxy(this.modal.setEnter, this));
				...
				{
					this.modal.tabs = this.$modal.find('.richeditorio-modal-tab');
					...
					this.modal.$tabsBox = $('<div id="richeditorio-modal-tabber" />');
				...
				showTab: function(e)
				{
					...
					$('#richeditorio-modal-tabber').find('a').removeClass('active');
					$el.addClass('active');
				...
				build: function()
				{
					...
					this.$modalBox = $('<div id="richeditorio-modal-box"/>').hide();
					this.$modal = $('<div id="richeditorio-modal" role="dialog" />');
					this.$modalHeader = $('<div id="richeditorio-modal-header" />');
					this.$modalClose = $('<button type="button" id="richeditorio-modal-close" aria-label="' + this.lang.get('close') + '" />').html('&times;');
					this.$modalBody = $('<div id="richeditorio-modal-body" />');
				...
				{
					this.$modalOverlay = $('<div id="richeditorio-modal-overlay">').hide();
					$('body').prepend(this.$modalOverlay);
				},
				enableEvents: function()
				{
					this.$modalClose.on('mousedown.richeditorio-modal', $.proxy(this.modal.close, this));
					$(document).on('keyup.richeditorio-modal', $.proxy(this.modal.closeHandler, this));
					this.core.editor().on('keyup.richeditorio-modal', $.proxy(this.modal.closeHandler, this));
					this.$modalBox.on('click.richeditorio-modal', $.proxy(this.modal.close, this));
				},
				disableEvents: function()
				{
					this.$modalClose.off('mousedown.richeditorio-modal');
					$(document).off('keyup.richeditorio-modal');
					this.core.editor().off('keyup.richeditorio-modal');
					this.$modalBox.off('click§.richeditorio-modal');
					$(window).off('resize.richeditorio-modal');
				},
				closeHandler: function(e)
				{
					if (e.which !== this.keyCode.ESC)
				...
				close: function(e)
				{
					if (e)
					{
						if ($(e.target).attr('id') !== 'richeditorio-modal-button-cancel' && e.target !== this.$modalClose[0] && e.target !== this.$modalBox[0])
						{
							return;
						}
					...
					this.$modalOverlay.richeditorioAnimation('fadeOut', { duration: 0.4 }, $.proxy(function()
					{
						this.$modalOverlay.remove();

					}, this));

					this.$modal.richeditorioAnimation('fadeOut', {
		...
		observe: function()
		{
			return {
				...
				buttons: function(e, btnName)
				{
					...
					if (!this.utils.isRicheditorioParent(current))
					{
						return;
					}
					...
					// {
                     //    console.log(this.utils.isCurrentOrParent(['table', 'pre', 'blockquote', 'li', 'figure']));
	    				// if (this.utils.isCurrentOrParentHeader() || this.utils.isCurrentOrParent(['table', 'pre', 'blockquote', 'li', 'figure'])) this.button.disable('horizontalrule');
					...
					$.each(this.opts.activeButtonsStates, $.proxy(function(key, value)
					{
						...
						if (parentEl.length !== 0 && !this.utils.isRicheditorioParent(parentEl))
						{
							return;
						}

						if (!this.utils.isRicheditorioParent(currentEl))
						{
							return;
						}
				...
				dropdowns: function()
				{
					...
					var $current = $(this.selection.current());
					var isRicheditorio = this.utils.isRicheditorioParent($current);
					...
					$.each(this.opts.observe.dropdowns, $.proxy(function(key, value)
					{
						...
						if (($current.closest(element).length > 0 && isRicheditorio) || (element === 'a' && finded !== 0))
						{
                ...
                images: function()
                {
                    ...
                    {
                        this.core.editor().find('img:not(.imageSrc)').each($.proxy(function (i, img) {
                            if (img.parentNode.tagName !== 'FIGURE' && !img.parentNode.classList.contains('imageFigure')) {
                                var $fig = this.figure.get('image', img.src);
                                $(img).replaceWith($fig);
                            }
                            else
                            {
                ...
                figures: function()
                {
                    if (this.opts.imageEditable)
                    {
                        this.core.editor().addClass('richeditorio-figure-layer-edit');
                        this.core.editor().find('figure.'+this.opts.classPrefix+'-figure').each($.proxy(function(i, figure)
                        {
                            if ($(figure).hasClass('imageFigure'))
                            {
                                this.figure.setEditable($(figure));
                            }
                        }, this));
                    }
					if (this.opts.embedEditable)
					{
						this.core.editor().addClass('richeditorio-figure-layer-edit');
						this.core.editor().find('figure.'+this.opts.classPrefix+'-figure').each($.proxy(function(i, figure)
						{
							if ($(figure).hasClass('embedFigure'))
							{
								this.figure.setEditable($(figure));
							}
						}, this));
					...
					if (this.opts.linkTooltip)
					{
    					this.core.editor().find('a').each($.proxy(function(i, s)
    					{
        					...
        					if ($link.data('cached') !== true)
        					{
            					...
            					$link.on('touchstart.richeditorio.' + this.uuid + ' click.richeditorio.' + this.uuid, $.proxy(this.observe.showTooltip, this));
        					}
				...
				showTooltip: function(e)
				{
					...
					var tooltip = $('<span class="richeditorio-link-tooltip"></span>');
					...
					var aLink = $('<a href="' + $link.attr('href') + '" target="_blank" />').html(href).addClass('richeditorio-link-tooltip-action');
					var aEdit = $('<a href="#" />').html(this.lang.get('edit')).on('click', $.proxy(this.link.show, this)).addClass('richeditorio-link-tooltip-action');
					var aUnlink = $('<a href="#" />').html(this.lang.get('unlink')).on('click', $.proxy(this.link.unlink, this)).addClass('richeditorio-link-tooltip-action');
					...
					this.core.editor().on('touchstart.richeditorio.' + this.uuid + ' click.richeditorio.' + this.uuid, $.proxy(this.observe.closeTooltip, this));
					$(document).on('touchstart.richeditorio.' + this.uuid + ' click.richeditorio.' + this.uuid, $.proxy(this.observe.closeTooltip, this));
				},
				...
				closeTooltip: function(e)
				{
					...
					else if ((target.tagName === 'A' && this.utils.isRicheditorioParent(target)) || $(target).hasClass('richeditorio-link-tooltip-action'))
					{
						return;
					}
					...
					this.core.editor().off('touchstart.richeditorio.' + this.uuid + ' click.richeditorio.' + this.uuid, $.proxy(this.observe.closeTooltip, this));
					$(document).off('touchstart.richeditorio.' + this.uuid + ' click.richeditorio.' + this.uuid, $.proxy(this.observe.closeTooltip, this));
				}
		...
		paragraphize: function()
		{
			return {
				...
				getSafes: function(html)
				{
                    ...
                    // deal with richeditorio selection markers
                    $div.find('span.richeditorio-selection-marker').each($.proxy(function(i,s)
                    {
                        this.paragraphize.z++;
                        this.paragraphize.safes[this.paragraphize.z] = s.outerHTML;
		...
		paste: function()
		{
			return {
				init: function(e)
				{
					...
					$(window).on('scroll.richeditorio-freeze', $.proxy(function()
					{
					...
					setTimeout($.proxy(function()
					{
						...
						$(window).off('scroll.richeditorio-freeze');
		...
		placeholder: function()
		{
			return {
				...
				show: function()
				{
					this.core.editor().addClass('richeditorio-placeholder');
				},
				...
				hide: function()
				{
					this.core.editor().removeClass('richeditorio-placeholder');
				},
				is: function()
				{
					return this.core.editor().hasClass('richeditorio-placeholder');
				},
				...
				enableEvents: function()
				{
					this.core.editor().on('keydown.richeditorio-placeholder.' + this.uuid, $.proxy(this.placeholder.enable, this));
				},
				disableEvents: function()
				{
					this.core.editor().off('.richeditorio-placeholder.' + this.uuid);
				},
				build: function()
				{
				...
				{
					var $style = $('<style />');
					$style.addClass('richeditorio-placeholder-style-tag');
					$style.html('#' + this.core.id() + '.richeditorio-placeholder::after ' + this.placeholder.getPosition());
				...
				destroy: function()
				{
					...
					$('.richeditorio-placeholder-style-tag').remove();
				}
			};
		},
		...
		progress: function()
		{
			return {
				...
				show: function()
				{
					...
					{
						this.progress.build();
						this.progress.$box.richeditorioAnimation('fadeIn');
					}
					else
					{
				...
				hide: function()
				{
					if (this.progress.is())
					{
						this.progress.$box.richeditorioAnimation('fadeOut', { duration: 0.35 }, $.proxy(this.progress.destroy, this));
					}
				},
				...
				build: function()
				{
					...
					this.progress.$box = $('<div id="richeditorio-progress" />');
			...
			return {
				...
				isRicheditorio: function()
				{
					...
					if (range !== null)
					{
						...
						if ($(el).hasClass('richeditorio-in') || $(el).parents('.richeditorio-in').length !== 0)
						{
							return true;
						}
					}
				...
				block: function(node)
				{
					...
					while (node)
					{
						if (this.utils.isBlockTag(node.tagName))
						{
							return ($(node).hasClass('richeditorio-in')) ? false : node;
						}
				...
				element: function(node)
				{
					...
					while (node)
					{
						if (node.nodeType === 1)
						{
							if ($(node).hasClass('richeditorio-in'))
							{
								return false;
							}
				...
				nodes: function(tag)
				{
					...
					$.each(nodes, function(i,s)
					{
						if (s)
						{
							...
							if ($(s).hasClass('richeditorio-script-tag') || $(s).hasClass('richeditorio-selection-marker'))
							{
								return;
							}
		...
		toolbar: function()
		{
			return {
				...
				createContainer: function()
				{
					return $('<ul>').addClass('richeditorio-toolbar').attr({ 'id': 'richeditorio-toolbar-' + this.uuid, 'role': 'toolbar' });
				},
				append: function()
				{
					if (this.opts.toolbarExternal)
					{
						this.$toolbar.addClass('richeditorio-toolbar-external');
				...
				setOverflow: function()
				{
					...
					{
    					this.$toolbar.addClass('richeditorio-toolbar-overflow');
    				}
				},
				setFixed: function()
				{
					...
					setTimeout($.proxy(function()
					{
						...
						if (this.detect.isDesktop())
						{
							$(this.opts.toolbarFixedTarget).on('scroll.richeditorio.' + this.uuid, function()
							{
				...
				{
					// tolerance 0 if richeditorio in the hidden layer
					var tolerance = 0;
				...
				observeScrollEnable: function(scrollTop, boxTop)
				{
					...
					if (scrollTop > end)
					{
						$('.richeditorio-dropdown-' + this.uuid + ':visible').hide();
					}

					this.toolbar.setDropdownsFixed();

					this.$toolbar.css('visibility', (scrollTop < end) ? 'visible' : 'hidden');
					$(window).on('resize.richeditorio-toolbar.' + this.uuid, $.proxy(this.toolbar.observeScrollResize, this));
				},
				observeScrollDisable: function()
				{
					...
					$(window).off('resize.richeditorio-toolbar.' + this.uuid);
				},
				...
				{
					var self = this;
					$('.richeditorio-dropdown-' + this.uuid).each(function()
					{
						var $el = $(this);
		...
		upload: function()
		{
			return {
				init: function(id, url, callback)
				{
					...
					this.upload.url = url;
					this.upload.$el = $(id);
					this.upload.$droparea = $('<div id="richeditorio-droparea" />');

					this.upload.$placeholdler = $('<div id="richeditorio-droparea-placeholder" />').text(this.lang.get('upload-label'));
					...
					this.upload.$droparea.off('richeditorio.upload');
					this.upload.$input.off('richeditorio.upload');

					this.upload.$droparea.on('dragover.richeditorio.upload', $.proxy(this.upload.onDrag, this));
					this.upload.$droparea.on('dragleave.richeditorio.upload', $.proxy(this.upload.onDragLeave, this));

					// change
					this.upload.$input.on('change.richeditorio.upload', $.proxy(function(e)
					{
                        e = e.originalEvent || e;
					...
					// drop
					this.upload.$droparea.on('drop.richeditorio.upload', $.proxy(function(e)
					{
						e.preventDefault();
		...
		utils: function()
		{
			return {
				...
				// parents detection
				isRicheditorioParent: function(el)
				{
					if (!el)
					{
						return false;
					...
					if ($(el).parents('.richeditorio-in').length === 0 || $(el).hasClass('richeditorio-in'))
					{
						return false;
					}
				...
				{
					this.core.editor().addClass('richeditorio-relative');
				},
				// scroll
				...
				measureScrollbar: function()
				{
					...
					scrollDiv.className = 'richeditorio-scrollbar-measure';
				...
				enableBodyScroll: function()
				{
					...
					$('body').remove('richeditorio-scrollbar-measure');
				},
	...
	$(window).on('load.tools.richeditorio', function()
	{
		$('[data-tools="richeditorio"]').richeditorio();
	});

	// constructor
	Richeditorio.prototype.init.prototype = Richeditorio.prototype;

})(jQuery);

(function($)
{
	$.fn.richeditorioAnimation = function(animation, options, callback)
	{
		return this.each(function()
		{
			new richeditorioAnimation(this, animation, options, callback);
		});
	};

	function richeditorioAnimation(element, animation, options, callback)
	{
		// default
		var opts = {
			duration: 0.5,
	...
	richeditorioAnimation.prototype = {
		...
		{
			this.$element.addClass('richeditorio-animated').css('display', '').removeClass('hide');
		...
		clean: function()
		{
			this.$element.removeClass('richeditorio-animated');
